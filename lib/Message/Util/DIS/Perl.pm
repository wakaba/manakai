...
'Removing module <%p (name => {http://suika.fam.cx/~wakaba/archive/2005/manakai/Util/DIS#uri});> from database...',
'http://suika.fam.cx/~wakaba/archive/2005/manakai/Util/Error/Core/textFormatter',
'Message::Util::Error::formatter',
'sev',
'1',
...
'http://suika.fam.cx/~wakaba/archive/2005/manakai/Util/DIS#DPG'
 => '1.0'});
      my $dpgdoc = $dpgparser->parse_string
                      ($val->
value
, $val);
...
get_property_text
                                ...
                                (
'http://suika.fam.cx/~wakaba/archive/2004/dom/feature#version'
, 
undef
);
          $ifpoint += $fver;
...
value
, 3;
    my $parent_resource = $disResource->parent_resource;
    my $uri = $qname eq ''
        ? ($parent_resource->parent_resource or $parent_resource)->uri
        : $self->
qname_to_uri
...
1
;
      $update_read{$target_module_uri} = 1;
...
;
  ...
  for my $mod_name (@$modules) {
    unless (defined $db->{modDef}->{$mod_name}) {
      $update_read{$mod_name} = 1;
    }
  }
